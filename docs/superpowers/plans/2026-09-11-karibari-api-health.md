# packages/api (/health) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 1PR1worker シリーズ第2弾として、`packages/api` に `/health` のみを持つ最小 Worker 土台を追加し、CI matrix に `api` 行を加える（mcp wave の踏襲）。

**Architecture:** mcp wave（PR #1）と同一形状。D1/R2 バインド・API ルート・viewer 同居は対象外（後続 wave）。DB 系依存（drizzle-orm 等）は `/health` に不要なので入れない（YAGNI）。

**Tech Stack:** 変更なし（Hono 4.13.7、`@cloudflare/vitest-plugin` 1.1.2、`vitest` 4.1.11、`wrangler` 4.127.1 は catalog 済み）。

**Spec:** `docs/superpowers/specs/2026-09-09-overall-arch-design.md`（Api＋Static 同居、3デプロイ単位）、`docs/superpowers/specs/2026-09-09-interfaces-design.md`（公開 API 一覧 — 本 PR の対象外、将来の配線先）、`docs/superpowers/specs/2026-09-09-tech-stack-design.md`（Hono、vitest-plugin）。
...
- `bun install` の後は必ず `bun dedupe` を走らせる（mcp wave の教訓 — install が緩いレンジを最新解決し `bun dedupe --check` が落ちる）。
...
- 本 plan は検証まで。commit / push / PR 作成は user の指示があるまで行わない。

---

### Task 1: packages/api workspace 本体＋knip 登録

**Files:**
- Create: `packages/api/package.json`
- Create: `packages/api/tsconfig.json`
- Create: `packages/api/wrangler.jsonc`
- Create: `packages/api/src/entry.ts`
- Create: `packages/api/src/server.ts`
- Create: `packages/api/src/routes/health.ts`
- Create: `packages/api/vitest.config.ts`
- Create: `packages/api/test/health.spec.ts`
- Generate: `packages/api/worker-configuration.d.ts`（commit する）
- Modify: `knip.config.ts`（`packages/api` エントリ追加）
...
`packages/api/package.json`（`@karibari/mcp` の api 版。hono は catalog 参照）：
...
`packages/api/wrangler.jsonc`（`name` だけ api。`secrets` の why コメント付き）：
...
`packages/api/test/health.spec.ts`：
...
test("GET /health is 200 with a fixed liveness body", async () => {
  const response = await exports.default.fetch(new Request("http://api/health"));
  expect(response.status).toBe(200);
  expect(await response.json()).toEqual({ message: "ok" });
});

test("anything else is 404", async () => {
  const response = await exports.default.fetch(new Request("http://api/"));
...
Run: `bun dedupe --no-progress --no-summary` then `bun run --cwd packages/api cf-typegen`
Expected: `packages/api/worker-configuration.d.ts` が生成される。`bun dedupe --check` が通る。

- [x] **Step 5: テストと型検査が通ることを確認する**

Run: `bun run --cwd packages/api test` then `bun run --cwd packages/api type-check`
...
- [x] **Step 2: api の3ステップが通ることを確認する**

Run: `bun run --cwd packages/api cf-typegen --check` then `bun run --cwd packages/api test` then `bun run --cwd packages/api wrangler deploy --dry-run --outdir "$RUNNER_TEMP/api-dist"`
Expected: すべて PASS。
